...
# -*-coding:utf-8 -*-
"""
writr by Awen in 2017.7
产生带有各种噪声的图片群,低SNR,云,类线状缺陷,宇宙射线
running python3.5
"""
...
def validateParam(funcName,distribName,runDistribName,distribParamsName,paramName,param,conditionStr):
	...
	for nn in range(0,len(conditionStr)):
		...
		if eqCond=={'<'}:
			...
			if operator.eq(conditionStr[2:],'integer')==0:
...
def ccd_photosensor_lightnoises(Cccd,Uin):
	if operator.eq('CMOS',Cccd.SensorType)==0:
...
class Adnoise:
	...
	def adcloud(self,f,n):
		...
		fx = np.array(range(int(-N/2),int(M/2)))*del_f
...
if __name__ == '__main__':
	'''
    	...
    	'''        
	rec = []
	path=r'/home/awen/NGC/*.fit*'
	if __name__== '__main__':
		rec = glob.glob(path)
		rec.sort() 
	print "There are",len(rec),"pairs of images under this folder"
	#for num in range(len(rec)):
	for num in range(100):
		hh=fits.open(rec[num])
		Uin=hh[0].data
		"""
		Uin=Uin.astype(np.float64)
		Uin=5*(Uin-np.min(Uin))/np.max(Uin)  #归一化到0-1之间
		#说不清为什么,这里用5, SNR会降低
		fM=len(Uin)
		fN=len(Uin[0])
		...
		f=AD.adcloud(f, random.uniform(-5,5)) #在[-10,10]取一个随机数作为阈值
		...
		showlist.writeto('/home/awen/桌面/CCD_modpicture/line2/line'+num_A+'.fits')
